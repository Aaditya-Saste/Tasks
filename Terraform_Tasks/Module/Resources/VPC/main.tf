resource "aws_vpc" "this_vpc" {
  cidr_block = var.this_vpc_cidr_block  //"10.0.0.0/16"
  tags = {
...
resource "aws_subnet" "this_subnet_pub" {
  ...
  cidr_block = var.this_subnet_pub_cidr_block  //"10.0.0.0/17"
...
resource "aws_subnet" "this_subnet_private1" {
    vpc_id = aws_vpc.this_vpc.id
    cidr_block = var.this_subnet_private1_cidr_block  //"10.0.128.0/19"
...
resource "aws_subnet" "this_subnet_private2" {
  vpc_id = aws_vpc.this_vpc.id
  map_public_ip_on_launch = var.this_subnet_private_map_ip //false
  cidr_block = var.this_subnet_private2_cidr_block  //"10.0.192.0/20"
  tags = {
